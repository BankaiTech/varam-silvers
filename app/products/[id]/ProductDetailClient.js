...
export default function ProductDetailClient({ productId }) {
  ...
  const product = {
    ...
    ...(parseInt(productId) > 7 && parseInt(productId) <= 16 ? {} : { weightGrams: getProductWeight(productId) }),
    ...
    category: parseInt(productId) <= 7 ? 'Chains' : 
              parseInt(productId) <= 16 ? 'Earrings' :
              parseInt(productId) <= 20 ? 'Bracelet' :
              parseInt(productId) <= 30 ? 'Kada' : 'Nazariya',
    materials: {
    ...
    specifications: {
      ...
      ...(parseInt(productId) > 7 && parseInt(productId) <= 16 ? {} : { 'Weight': `${getProductWeight(productId)} grams (approx)` }),
